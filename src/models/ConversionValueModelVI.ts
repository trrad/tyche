  ...
  export class ConversionValueModelVI {
    ...
    getSummary(): string {
      ...
      for (const [name, summary] of this.summaries) {
        ...
        lines.push(`  Conversions: ${summary.conversions} (${(summary.conversions / summary.n * 100).toFixed(1)}%)`);
    ...
    async analyze(options: VIAnalysisOptions = {}): Promise<ConversionValuePosterior> {
      const modelType = options.modelType === 'auto' 
        ...
        : options.modelType || 'beta-binomial';
      ...
      for (const [variantName, data] of this.variants) {
        ...
        const conversionResult = await this.viEngine.fit(
          'beta-binomial', 
          conversionInput,
          {
        ...
        const convSamples = Array.from({ length: 1000 }, () => 
        ...
        if (summary.convertedValues.length > 0) {
          ...
          const valueSamples = Array.from({ length: 1000 }, () => 
            valueResult.posterior.sample()[0]
          );
    ...
    private computeEffectDrivers(
      ...
      for (let i = 1; i < variants.length; i++) {
        ...
        const interaction = (avgTreatmentConv - avgControlConv) * (avgTreatmentValue - avgControlValue);
    ...
    private aggregateVIDiagnostics(viResults: Map<string, VIResult>): any {
      ...
      // Average diagnostics
      const avgIterations = allResults.reduce((sum, r) => sum + r.diagnostics.iterations, 0) / allResults.length;
      const allConverged = allResults.every(r => r.diagnostics.converged);
      const avgELBO = allResults.reduce((sum, r) => sum + (r.diagnostics.finalELBO || 0), 0) / allResults.length;
