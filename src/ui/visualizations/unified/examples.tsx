  ...
  useEffect(() => {
    ...
    const calculateDiagnostics = async () => {
      try {
        ...
        const obsVar =
          observedData.reduce((sum, x) => sum + Math.pow(x - obsMean, 2), 0) / observedData.length;
        const predVar =
          samples.reduce((sum, x) => sum + Math.pow(x - predMean, 2), 0) / samples.length;
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span className="ml-2 font-mono">
                    {(diagnostics.relativeBias * 100).toFixed(1)}%
                  </span>
                </div>
              </div>
              ...
              <div className="border-t pt-3 mt-3">
                <div className="text-sm">
                  ...
                  <span className="ml-2 text-gray-500">
                    (
                    {diagnostics.ksStatistic < 0.05
                      ? 'Good fit'
                      : diagnostics.ksStatistic < 0.1
                        ? 'Moderate fit'
                        : 'Poor fit'}
                    )
                  </span>
...
export const ModelComparisonExample: React.FC<{
  ...
  if (observedData) {
    distributions.unshift({
      id: 'observed',
      label: 'Observed',
      posterior: observedData,
      color: '#1f2937',
      metadata: { isObserved: true, variantIndex: 0 },
    } as any); // Cast to any since observed data needs special properties
  }
...
export const TimeSeriesDistributionExample: React.FC<{
  ...
  const colorScale = d3.scaleSequential(d3.interpolateBlues).domain([0, timePoints.length - 1]);
  ...
  return (
    <div>
      ...
      <div className="mt-4 text-sm text-gray-600">
        ...
        {thresholds.map((t) => (
          <p key={t}>
            P(effect {'>'} {(t * 100).toFixed(0)}%) = [calculated from samples]
          </p>
        ))}
      </div>
    </div>
