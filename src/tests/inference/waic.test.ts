...
function generateBinomialData(successes: number, trials: number): { successes: number, trials: number } {
...
function debugMultimodalityDetection(data: number[]) {
  ...
  const bimodalityCoefficient = (skewness ** 2 + 1) / 
  ...
  return {
    n, mean, std, cv, skewness, kurtosis, bimodalityCoefficient,
    p25, p50, p75, gap1, gap2, meanGap, hasGaps,
...
describe('WAIC basic functionality', () => {
  it('computes WAIC for a simple beta-binomial model', async () => {
    ...
    const waicResult = await ModelSelectionCriteria.computeWAIC(result.posterior, [data], 'beta-binomial');
  ...
  it('computes WAIC for lognormal model', async () => {
    ...
    const waicResult = await ModelSelectionCriteria.computeWAIC(result.posterior, data, 'lognormal');
    expect(Number.isFinite(waicResult.waic)).toBe(true);
    expect(Number.isFinite(waicResult.elpd)).toBe(true);
    expect(Number.isFinite(waicResult.pWaic)).toBe(true);
    expect(Number.isFinite(waicResult.logLikelihood)).toBe(true);
  ...
  it('tests different sample sizes', async () => {
    ...
    const waicResult1 = await ModelSelectionCriteria.computeWAIC(result.posterior, [data], 'beta-binomial');
    
    // Test with more samples
    ModelSelectionCriteria.WAIC_SAMPLES = 2000;
    const waicResult2 = await ModelSelectionCriteria.computeWAIC(result.posterior, [data], 'beta-binomial');
...
describe('ModelRouter WAIC Integration', () => {
  it('uses WAIC for component selection in auto mode', async () => {
    ...
    const result = await engine.fit('auto', { data: multimodalData }, {
      useWAIC: true,
  ...
  it('works with different data types', async () => {
    ...
    const unimodalResult = await engine.fit('auto', { data: unimodalData }, {
      useWAIC: true,
      returnRouteInfo: true
    });
    ...
    const multimodalData = generateMultimodalData();
    const multimodalResult = await engine.fit('auto', { data: multimodalData }, {
      useWAIC: true,
      returnRouteInfo: true
    });
...
describe('Debug: Multimodality Detection', () => {
...
describe('Compound Model WAIC Integration', () => {
  it('selects optimal compound model using WAIC', async () => {
    ...
    // Test auto mode with WAIC enabled
    const result = await engine.fit('auto', { data: compoundData }, {
      useWAIC: true,
      returnRouteInfo: true
    });
  ...
  it('falls back to heuristics for small compound datasets', async () => {
    ...
    const result = await engine.fit('auto', { data: smallCompoundData }, {
      useWAIC: true,
      returnRouteInfo: true
    });
