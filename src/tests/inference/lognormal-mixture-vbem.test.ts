...
describe('LogNormalMixtureVBEM', () => {
  ...
  describe('Basic functionality', () => {
    ...
    it('should have monotonically increasing ELBO', async () => {
      ...
      // Debug: Log ELBO progression
      console.log('\n=== ELBO History Debug ===');
      console.log('Total iterations:', elboHistory.length);
      console.log('Initial ELBO:', elboHistory[0]);
      console.log('Final ELBO:', elboHistory[elboHistory.length - 1]);
      console.log('Overall change:', elboHistory[elboHistory.length - 1] - elboHistory[0]);
      ...
      for (let i = 1; i < elboHistory.length; i++) {
        ...
        if (increase < 0) {
          decreases.push(increase);
          console.log(`Iteration ${i}: ELBO decreased by ${increase}`);
        }
      }

      if (decreases.length > 0) {
        console.log('\nTotal decreases:', decreases.length);
        console.log('Largest decrease:', Math.min(...decreases));
        console.log('Average decrease:', decreases.reduce((a, b) => a + b, 0) / decreases.length);
      }
