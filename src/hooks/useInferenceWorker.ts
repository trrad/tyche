...
import type { StandardData } from '../core/data/StandardData';

// Temporary types until hook is updated to new architecture
type ModelType = string;
type DataInput = any;
type CompoundDataInput = any;
import {
  PosteriorProxy,
  CompoundPosteriorProxy,
  PosteriorSummary,
} from '../workers/PosteriorProxy';
    ...
    async (
      ...
      try {
        if (workerRef.current) {
          ...
          const result = await new Promise<InferenceResult & { waicInfo?: any; routeInfo?: any }>(
            (resolve, reject) => {
              ...
              const handleMessage = (event: MessageEvent) => {
                ...
                switch (event.data.type) {
                  ...
                  case 'result': {
                    ...
                    const { posteriorIds, diagnostics, summary, waicInfo, routeInfo } =
                      event.data.payload;
          ...
          // Fallback to main thread - disabled until hook is updated
          console.log('🔄 Main thread fallback not available');
          throw new Error('Worker not available and main thread fallback disabled');

          // TODO: Update to use ModelRouter directly
          // const { config, engine } = await ModelRouter.route(data, options);
          // return await engine.fit(data, config, options);
        }
      } catch (err: any) {
