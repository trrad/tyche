...
import {
  ...
  GeneratedDataset,
  NoiseLevel,
} from '../src/tests/utilities/synthetic/DataGenerator';
...
import { ComponentSelector } from '../src/ui/components/ComponentSelector';
...
// MODEL_DESCRIPTIONS removed - now in ModelSelector
...
return { ...base, data: new DataGenerator(seed).applyNoiseLevel(base.data, '${noiseLevel}') };`,
  ...
  const formatValue = useCallback(
    (value: number) => {
      if (!modelConfig) return value.toFixed(3);

      const modelType = modelConfig.type || modelConfig.valueType;
      if (modelType === 'beta' || modelConfig.structure === 'compound') {
        return `${(value * 100).toFixed(1)}%`;
      } else if (modelType === 'lognormal' || modelType === 'gamma') {
  ...
  const getParameterLabel = (config: ModelConfig | null): string => {
    if (!config) return 'Parameter';

    if (config.type === 'beta' || config.structure === 'compound') {
      return 'Conversion Rate';
    } else if (config.type === 'lognormal' || config.valueType === 'lognormal') {
      return 'Revenue';
  ...
  const runInference = useCallback(async () => {
    ...
    try {
      // Convert data to StandardData format
      let standardData: StandardData;

      if (Array.isArray(dataToAnalyze)) {
        if (
          dataToAnalyze.length > 0 &&
          typeof dataToAnalyze[0] === 'object' &&
          'converted' in dataToAnalyze[0]
        ) {
          // User-level data with conversion
          const users = dataToAnalyze as UserLevelData[];
          standardData = StandardDataFactory.fromUserLevel(users);
        } else {
          // Simple continuous data
          standardData = StandardDataFactory.fromContinuous(dataToAnalyze as number[]);
        }
      } else if (dataToAnalyze.successes !== undefined && dataToAnalyze.trials !== undefined) {
        // Binomial data
        standardData = StandardDataFactory.fromBinomial(
          dataToAnalyze.successes,
          dataToAnalyze.trials
        );
      } else {
      ...
      setError(`Inference failed: ${error instanceof Error ? error.message : String(error)}`);
      setInferenceResult(null);
    } finally {
        ...
        <div className="space-y-4">
          <div className="relative">
            <select
              value={
                selectedDataSource
                  ? dataSources.findIndex((ds) => ds.name === selectedDataSource.name)
                  : ''
              }
              onChange={(e) => {
          ...
          syntheticCode={
            selectedDataSource?.getCode
              ? selectedDataSource.getCode({
                  ...
                  sampleSize: useCustomSampleSize
                    ? sampleSize
                    : selectedDataSource.category === 'compound'
                      ? 2000
                      : 1000,
  ...
  const ResultsDisplay = () => {
    ...
    if (isCompound) {
      ...
      ppcHeadline = `Beliefs about ${getParameterLabel(modelConfig).toLowerCase()}`;
    }
      ...
      <div className="space-y-6">
              ...
              <UnifiedDistributionViz
                ...
                display={{
                  ...
                  binCount: (() => {
                    ...
                    const obsData =
                      isCompound && typeof visualizationData[0] === 'object'
                ...
                xLabel={isCompound ? 'Value (Converted Users)' : getParameterLabel(modelConfig)}
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <VisualizationErrorBoundary>
            <AsyncPosteriorSummary
              ...
              modelType={modelConfig?.type || modelConfig?.valueType || 'auto'}
            />
          </VisualizationErrorBoundary>
        </div>
            ...
            <VisualizationErrorBoundary>
              <UnifiedDistributionViz
                distributions={[
                  {
                    id: 'posterior',
                    label: getParameterLabel(modelConfig),
                ...
                title={`${getParameterLabel(modelConfig)} Distribution`}
              />
            </VisualizationErrorBoundary>
          </div>
        )}
        ...
        {showComparison && routeInfo?.modelParams?.waicComparison && (
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <ModelComparisonView waicComparison={routeInfo.modelParams.waicComparison} />
          </div>
        )}
